 ...
 * ============================================================
 * 국세청 사업자상태 API는 상호명을 제공하지 않으므로, 민간 기업정보
 * DB(비즈노)로 상호명만 병렬 보완한다. **참고용**이며 휴/폐업 판정에는
 * 절대 사용하지 않는다(판정은 국세청 결과 기준 유지).
 *
 * 문서: https://docs.bizno.moneypin.biz  (무료 키: 1일 200건 제한)
 * 엔드포인트: https://api.bizno.net/api/fetch?key=KEY&gb=1&q=사업자번호&type=json
 *   - key : 발급 API 키 (환경변수 BIZNO_API_KEY)
 *   - gb  : 검색구분(1 = 사업자등록번호)
 *   - q   : 사업자등록번호(하이픈 제거)
 *   - type: json
 *   ※ 엔드포인트는 BIZNO_API_URL 로 덮어쓸 수 있음(신규 포털 대비)
 *
 * 설계 원칙(요구사항):
 *   - 상호명은 병렬 조회, 기존 검증 흐름에 영향 없음
 *   - 비즈노에 없는 번호 → 상호명 null("상호명 미확인"으로 표시)
...
const BIZNO_ENDPOINT = process.env.BIZNO_API_URL || 'https://api.bizno.net/api/fetch';
const TIMEOUT_MS = 3000;

// 응답 스키마가 API 버전에 따라 다를 수 있어, 상호명 후보 필드를 방어적으로 탐색.
const NAME_KEYS = [
  'company', 'companyName', 'company_nm', 'companyNm',
  'corpNm', 'bizNm', 'trueName', 'name', 'enterprise',
  '상호', '상호명', '회사명',
];
...
// 응답 객체에서 상호명으로 보이는 첫 문자열 필드를 재귀 탐색
